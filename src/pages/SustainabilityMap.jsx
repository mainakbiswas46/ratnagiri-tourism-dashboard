...
export default function SustainabilityMap() {
  ...
  const tabs = [
    { id: 'villages', label: 'Villages', icon: TreePine },
    { id: 'homestays', label: 'Homestays', icon: BedDouble },
    { id: 'eco', label: 'Eco & Water', icon: Droplets },
    { id: 'register', label: 'Register', icon: Plus },
    { id: 'pins', label: 'Custom Pins', icon: MapPin }
  ];
        ...
        <Marker key={home.id} position={[home.lat, home.lng]}>
          ...
          <Popup>
            <div className="text-center">
              ...
              <button 
                onClick={() => navigate(`/profile/${home.id}`)}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeTab === 'villages' && (
            <div className="animate-in slide-in-from-right-4 duration-300">
              <h2 className="text-lg font-bold text-slate-800 mb-4 border-l-4 border-orange-500 pl-3">Village Wise Tourist Locations</h2>
              <div className="space-y-4">
                {locations.map(loc => (
                  ...
                  >
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); navigate(`/profile/${loc.id}`); }}
            ...
            <div className="animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-4">
                {homestays.map(home => (
                  <div 
                    key={home.id}
                    onClick={() => handleFlyTo(home.lat, home.lng)}
                    className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-orange-500 hover:shadow-md cursor-pointer transition-all"
                  >
      ...
      <div className="flex-1 relative bg-slate-200" style={{ cursor: pinMode ? 'crosshair' : 'grab' }}>
        <MapContainer center={[17.7554, 73.1923]} zoom={11} className="w-full h-full z-0">
          <LayersControl position="topright">
            <LayersControl.BaseLayer checked name="Satellite Background">
              <TileLayer
                url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}"
                attribution="Tiles &copy; Esri &mdash; Source: Esri"
              />
            </LayersControl.BaseLayer>
            <LayersControl.BaseLayer name="Standard Vector View">
              <TileLayer
